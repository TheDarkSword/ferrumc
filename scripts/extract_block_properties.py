#!/usr/bin/env python3
"""Ask the game what each block state is like to break.

How hard a state is and whether the right tool is needed for it to drop anything both live on the
block's behaviour, set in code when the block is built. No report carries either.

Output is `assets/extracted/<version>/block_properties.json`, keyed by state id.

Usage:
    scripts/extract_block_properties.py
    scripts/extract_block_properties.py 26.1
"""
...
EXTRACTOR = REPO_ROOT / "scripts" / "extractor" / "BlockPropertyExtractor.java"
NATIVE = "26.2"
...
def main() -> None:
    ...
    out = REPO_ROOT / "assets" / "extracted" / args.version / "block_properties.json"
    ...
    print(f"{len(data)} block states -> {out.relative_to(REPO_ROOT)}")
