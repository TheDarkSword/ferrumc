#!/usr/bin/env python3
"""Ask the game what each item leaves behind when it is crafted with.

A bucket of milk in a cake recipe leaves the bucket. Which items do that is a field on the item, set
in code, and no report carries it.

Output is `assets/extracted/crafting_remainders.json`.

Usage:
    scripts/extract_crafting_remainders.py
    scripts/extract_crafting_remainders.py 26.1
"""
...
EXTRACTOR = REPO_ROOT / "scripts" / "extractor" / "CraftingRemainderExtractor.java"
NATIVE = "26.2"
...
def main() -> None:
    ...
    out = REPO_ROOT / "assets" / "extracted" / "crafting_remainders.json"
    ...
    print(f"{len(data)} items leave something behind -> {out.relative_to(REPO_ROOT)}")
