...
pub fn handle(
    ...
    for event in events.read() {
        ...
        let Ok((_, _, new_pos, new_rot, new_conn, new_data)) = player_query.get(new_player_entity)
        else {
            error!(
        ...
        // Whatever is already true of the new player — crouching, swimming, on fire — has to
        // reach everyone who is about to see them, not just whoever is watching when it changes.
        let new_player_metadata =
            EntityMetadataPacket::everything(VarInt::new(new_player_identity.short_uuid), new_data);

        let mut spawned_for_new_player = 0;
        let mut spawned_for_existing = 0;

        for (entity, identity, pos, rot, conn, data) in player_query.iter() {
            // Skip self
            ...
            let existing_player_metadata =
                EntityMetadataPacket::everything(VarInt::new(identity.short_uuid), data);
            if let Err(e) = new_conn.send_packet_ref(&existing_player_metadata) {
                error!(
                    "Failed to send existing player metadata to new player: {:?}",
                    e
                );
                continue;
            }
            ...
            if let Err(e) = conn.send_packet_ref(&new_player_metadata) {
                error!(
                    "Failed to send new player metadata to existing player: {:?}",
                    e
                );
                continue;
            }
